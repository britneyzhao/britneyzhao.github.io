...
In our model, we embedded two different models, one for titles and one for texts. This embedding creates an association between the words and their category of truth: either fake or not. Because of this, we can analyze how each word relates to being fake or not. In other words, we can see if some words appear more frequently in fake news articles than others. 
...
First, let's look at the results from the title embedding. To get the weights and vectorize the vocabulary, we'll use functions from Tensorflow like so:

```python
weights_title = model.get_layer('embedding_title').get_weights()[0] # get the weights from the embedding layer
...
weights_text = pca.fit_transform(weights_text)
```
...
```python
embedding_df_text = pd.DataFrame({
    'word' : vocab, 
    'x0'   : weights_text[:,0],
    'x1'   : weights_text[:,1]
})
...
```python
fig = px.scatter(embedding_df_title, 
                 x = "x0", 
                 y = "x1", 
                 size = list(np.ones(len(embedding_df_title))),
                 size_max = 2,
                 hover_name = "word")

fig.show()
```
{% include embed_title.html %}

Here, we can see the graph generally goes either to the left or the right. On the left, we have words like "attorney" and "think," whereas the right has words like "trump," "obama," and "president." From this, we can see that the more politically-driven articles tend to have more false information within them, whereas articles with more neutral titles tend to have little to no fake news. 

Let's perform the same analysis on the text! For this, we'll use the same exact process as we did for the titles: obtain the weights of the embedding layer, scale to two-dimensions, create the dataframe comparing the weights to the words, and plotting with `plotly`! All together, the code should look very similar to what we did above: 

```python
weights_text = model.get_layer('embedding_text').get_weights()[0]
...
fig2 = px.scatter(embedding_df_text, 
                 x = "x0", 
                 y = "x1", 
                 size = list(np.ones(len(embedding_df_text))),
                 size_max = 2,
                 hover_name = "word")

fig2.show()
```
{% include embed_text.html %}

While this graph looks quite similar in shape, we see it is a bit more center with a slightly clearer pattern. On the right, we have similar words like "trumps" and "obamas," whereas the left has words like "wonder" and "getty." The articles with more political content tend to have more false information in them, which would make sense in today's climate. Whereas, on the left, neutral words like "wonder" and "getty" have little political affiliation, and tend to warrent less false information in them. 
